...
export default function PersonalInfo() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!teamLogoUrl) {
        alert("No logo found for the selected team.");
        setLoading(false);
        return;
      }

      console.log("📤 Sending logo to n8n:", teamLogoUrl);

      const response = await fetch(LOGO_BACKGROUND_ENDPOINT, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          logo: teamLogoUrl,
      ...
      if (Array.isArray(result)) {
        imageBase64 = result[0]?.image_base64;
      ...
      if (imageBase64) {
        ...
        console.log("🖼️ Converted background image ready");

        navigate("/main", { state: { backgroundImage } });
      } else {
        console.error("❌ No image_base64 found in n8n response", result);
        alert("No image returned from n8n. Please check your workflow output.");
      }


    } catch (err) {
      console.error("❌ Error sending logo to n8n:", err);
      alert("Failed to send logo to n8n. Check console for details.");
    } finally {
      setLoading(false);
    }
  };
